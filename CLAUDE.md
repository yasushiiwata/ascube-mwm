...
17. **当日の身長・体重は、当日測定した値だけを送る。**
    ...
    ⚠ **`(0010,1020)` の単位は DICOM 規格上メートル。BRIDGE-Navi はセンチメートルで扱う。**
    cm→m の変換は BRIDGE-Navi ではなく **ascube-mwm 側の責務**（`WorklistEntry.PatientHeightCm` を
    受け取り、DICOM 送信直前に `/100.0` する）。BRIDGE-Navi 側で変換させると、変換漏れ・二重変換が
    ascube-mwm から検知できない（詳細: `docs/設計変更メモ_v2.1.md` §I）。

18. **依頼電文の項番3（個人番号・12桁）と項番4（受診番号・5桁）を取り違えない。**
    PatientID に入れるのは**項番3**。項番4 は毎回変わるので、入れると同じ人が
    毎年別人として登録され、骨密度の経年比較が壊れる。**1年後まで発覚しない。**

19. **文字コードの往復検証に fo-dicom の decoder を使わない（T0/alt4 で確認）。**
    `ISO_IR 13` を指定した状態で漢字を渡すと、fo-dicom は無警告で CP932 相当の
    バイト列を書き、自分ではそれを正しく読み戻す。**往復検証は合格してしまう。**
    判定は生バイトを直接検査する：
      `ISO_IR 13`  → 全バイトが 0x20〜0x7E または 0xA1〜0xDF。外れたら Suppressed
      `ISO_IR 192` → `UTF8Encoding(false, throwOnInvalidBytes: true)` で decode
    規格適合の最終判定は DCMTK（別実装）で行う。
