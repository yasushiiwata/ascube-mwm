...
## T1（2026-09-09実施）で確定した設計上の決定事項

- **`docs/開発計画_v2.md`（プロファイルスキーマの根拠文書、付録A・B相当）がリポジトリに存在しない。**
  git履歴を確認したが一度もコミットされていない。CLAUDE.md はこのファイルを「設計判断の根拠」として
  参照しているが、実体が無いため、T1 の `dataset.elements` DSL（`db:`/`const:`/`echo:`/`uid:study`/
  `coalesce:`/`auto:*`、SQ構造、`visibility`/`matching`/`charset` ブロックの具体的なキー名）は
  `docs/実装指示書.md` §3・§5-T6・§5-T7、`docs/設計変更メモ_v2.1.md` B-6、
  `docs/依頼電文マッピング表_v1.1.md` から再構築した**独自設計**である（ユーザー承認済み）。
  T2 以降で実データ・実運用と突き合わせて、命名やDSLの形が実態と合わなければ見直すこと。
  根拠: `src/Ascube.Mwm.Core/Config/`、`config/profiles/*.jsonc` のコメント。
